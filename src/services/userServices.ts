 ...
 */
export const insertSeedSuperAdmin = async (email: string, hash: string) => {
 ...
 */
function signAccess(user: { id: number; role: string; createdAt: string }) {
    return jwt.sign(
        { sub: user.id, role: user.role, createdAt: user.createdAt },
        config.auth.jwtAccessKey,
        { expiresIn: config.auth.accessExpires as string }
    );
}
 ...
 */
function signRefresh(userId: number, jti: string) {
    return jwt.sign(
        { sub: userId, jti },
        config.auth.jwtRefreshKey,
        { expiresIn: config.auth.refreshExpires as string }
    );
}
 ...
 */
export async function refreshAccess(oldRefresh: string) {
    let payload: jwt.JwtPayload;
    try {
        const decoded = jwt.verify(oldRefresh, config.auth.jwtRefreshKey);
        if (typeof decoded === "string") throw new HttpError("Invalid or expired refresh token", 401);
        payload = decoded;
    } catch {
    ...
    if (!user.is_active) throw new HttpError("Invalid credentials", 401);

    const hashedJti = crypto.createHash("sha256").update(String(payload.jti)).digest("hex");
    if (!rows[0] || rows[0].refresh_token !== hashedJti) throw new HttpError("Refresh token replay detected", 403);

    const newJti = crypto.randomUUID();
    const newRefreshToken = signRefresh(Number(payload.sub), newJti);
    ...
    return {
        accessToken: signAccess({ id: Number(payload.sub), role: rows[0].role, createdAt: rows[0].created_at }),
 ...
 */
export async function createStaff({ email, password, role, name, roleId }: { email: string; password: string; role: string; name: string; roleId?: number | null }, creatorId: number) {
 ...
 */
export async function updateUser(userData: { name: string; email: string; role: string; roleId?: number | null }, userId: number) {
    const { rows } = await query(
