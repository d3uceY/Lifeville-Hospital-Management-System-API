 ...
 */
export async function getOrCreateInvoice({ admissionId = null, visitId = null }: { admissionId?: number | null; visitId?: number | null }) {
...
export async function addItem({
  ...
  let invoiceId: number | null = invoiceIdParam;
  // Resolve invoice
  if (!invoiceId) {
  ...
  if (LOCKED_STATUSES.includes(inv.status)) throw new HttpError(`Cannot add items to a ${inv.status} invoice.`, 409);
 ...
 */
export async function deleteBillItem(itemId: number) {
  ...
  if (item.invoiceId) {
    ...
    const LOCKED_STATUSES = [INVOICE_STATUSES.PAID, INVOICE_STATUSES.PARTIAL, INVOICE_STATUSES.CANCELLED];
    if (inv && LOCKED_STATUSES.includes(inv.status)) throw new HttpError(`Cannot remove items from a ${inv.status} invoice.`, 409);
  }
...
export async function getServicePrice(name: string) {
  const [svc] = await db
...
export async function getServicePriceById(id: number) {
  const [svc] = await db
    .select({ price: services.price })
    .from(services)
 ...
 */
export async function getBillForAdmission(admissionId: number) {
  // Fetch admission
  ...
  const itemsWithRunning = allItems.map((item) => {
    const rawLineTotal = (item as { lineTotal?: string | number | null }).lineTotal;
    const lineTotal = rawLineTotal !== undefined && rawLineTotal !== null ? parseFloat(String(rawLineTotal)) : (item as { unitPrice: number; quantity: number }).unitPrice * (item as { unitPrice: number; quantity: number }).quantity;
    running += lineTotal;
 ...
 */
export async function getBillForVisit(visitId: number) {
 ...
 */
async function computeInvoiceTotal(invoiceId: number, invoiceRow: typeof invoices.$inferSelect | null = null) {
  ...
  const storedTotal = parseFloat(String(totRow?.total ?? 0));
...
export async function recordPayment({
  ...
  const [inv] = await db.select({ status: invoices.status }).from(invoices).where(eq(invoices.id, invoiceId)).limit(1);
  if (!inv) throw new HttpError("Invoice not found.", 404);
  if (inv.status === INVOICE_STATUSES.CANCELLED) throw new HttpError("Cannot record a payment against a cancelled invoice.", 409);
  if (inv.status === INVOICE_STATUSES.PAID) throw new HttpError("This invoice is already fully paid.", 409);
  ...
  const totalPaidSoFar = parseFloat(String(paidRow?.paid ?? 0));
...
let servicesCache: typeof import('../../drizzle/migrations/schema.js').services.$inferSelect[] | null = null;
 ...
 */
export async function listServices({ page, pageSize, category, search }: { page?: number; pageSize?: number; category?: string; search?: string } = {}) {
...
export async function getServiceCounts() {
  ...
  return rows.reduce<Record<string, number>>((acc, r) => { acc[r.category] = Number(r.total); return acc; }, {});
}
 ...
 */
export async function upsertService({ id = null, name, category, price, isVariablePrice = false }: { id?: number | null; name: string; category: string; price: number; isVariablePrice?: boolean }) {
  if (id) {
  ...
  const [existing] = await db
    ...
    .where(eq(services.name, name))
    .limit(1);
  if (existing) throw new HttpError(`A service named "${name}" already exists.`, 409, "DUPLICATE_SERVICE_NAME");
...
export async function deleteService(id: number) {
  const [svc] = await db
    ...
    .limit(1);
  if (!svc) return null;
  if (svc.isSystem) throw new HttpError("System services cannot be deleted.", 409, "SYSTEM_SERVICE");
  const [deleted] = await db
    .delete(services)
    .where(eq(services.id, id))
    .returning();
...
export async function getPatientInvoices(patientId: number) {
  ...
  const enriched = await Promise.all(allInvoices.map(async (inv) => {
    ...
    const totalPaid = parseFloat(String(paidRow?.paid ?? 0));
 ...
 * @param {number} invoiceId
 */
export async function getBillForManualInvoice(invoiceId: number) {
 ...
 */
export async function createPatientInvoice(patientId: number) {
 ...
 */
export async function getPatientBillingContext(patientId: number) {
...
export async function getPaginatedInvoices({ page = 1, pageSize = 10, search = "", status = "" } = {}) {
  ...
  const normalize = (v: string) => (typeof v === "string" && v.trim() !== "" && v !== "undefined" ? v.trim() : null);
